...
import Data.Map ( empty, Map, toList )
...
hokiToCoreExpr :: Core.LProg -> Hoki.Expr -> Core.Expr
...
hokiToCoreExpr (LProg tenv a b venv) (Hoki.Eabb name args exprs typesig) = let
    expr = case args of
        [] -> throw Unimplemented
        xs -> foldr Core.ELambda (Core.ELambda (last xs) (head exprs)) (init xs)
    -- tenv' = insert name ()
    tenv' = tenv
    venv' = insert name expr
